...
def UDNLLLoss(true_label, predicted_scores, lengths):
    ...
    predicted_scores.view(len(true_label[0])*len(true_label[0]), -1)
    _loss = torch.tensor(0, dtype=torch.float).to(device)

    for i, lab in enumerate(true_label):
        _loss = torch.sum(-1.0*predicted_scores[torch.from_numpy(np.arange(lengths[i]))*lengths[i].add(torch.tensor(lab[:lengths[i]]))][:, i]).add(_loss)
...
class DependencyDataset(Dataset):
    ...
    def __init__(self, words_dict, tags_dict, file_path, padding=False, word_embeddings=None):
        ...
        super().__init__()
        self.file_path = file_path
        self.data_reader = DataReader(self.file_path, words_dict, tags_dict)
    ...
    def convert_sentences_to_dataset(self, padding):
        ...
        if padding:
            ...
            all_sentence_tags_idx = torch.tensor(sentence_tags_idx_list, dtype=torch.long, requires_grad=False)
            all_sentence_labels_idx = torch.tensor(sentence_labels_idx_list, dtype=torch.long, requires_grad=False)
