...
import numpy as np
from BasicModel import LSTMEncoder

torch.manual_seed(0)


def generate_comp_tagged_file_basic(model_path, target_path, path_comp):
    """
    ...
    """
    torch.manual_seed(0)
    words_dict, pos_dict = BasicModel.get_vocabs(trained_on_path_list)
    comp = BasicModel.DependencyDataset(words_dict, pos_dict, path_comp, padding=True, competition=True)
    comp_data_loader = BasicModel.DataLoader(comp, batch_size=1, shuffle=False, num_workers=0)
...
def generate_comp_tagged_file_advance(model_path, target_path, path_comp):
    """
    Generates a tagged version of the competition file.
    Args:
        model_path (str): The path to the chosen model's pth file.
...
if __name__ == '__main__':
    ...
    basic_model_path = 'encoder06_30_2020_02_16_05.pth'
    advanced_model_path = 'encoder06_30_2020_01_22_43.pth'

    generate_comp_tagged_file_basic(basic_model_path, path_comp_m1_labeled, path_comp)
    # TODO: bad practice, change it when you have time.
    from AdvancedModel import LSTMEncoder
    generate_comp_tagged_file_advance(advanced_model_path, path_comp_m2_labeled, path_comp)
